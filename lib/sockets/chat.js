...
var connectionHandler = function(socket) {
  ...
  socket.on('subscribe', function(data) {
    ...
    Room.getOrCreate(data.roomId, function(room) {
      ...
      Room.save(room, {conflict: 'update'}).then(function(room) {
        ...
        r.table('authors')
          .innerJoin(room[0].presentAuthors, function(author, presentAuthor) {
  ...
  socket.on('disconnect', function (disconnect) {
    ...
    Room.get(connectionState.roomId).then(function(room) {
      var index = room.presentAuthors.indexOf(connectionState.authorId);
      room.presentAuthors.splice(index);
      Room.save(room, {conflict: 'update'}).then(function(room) {
