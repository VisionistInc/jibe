...
// flag the latest revision
router.post('/:roomId/flag', function(req, res) {
  // for whatever reason, after doing max aggregation, can't update?
  var idOfMaxVersion = r.table('jibe_ops')
    .filter({ name: req.params.roomId })
    .max('v')
    .getField('id');
  ...
  .error(function(error) {
    console.error('Error flagging operation', error);
